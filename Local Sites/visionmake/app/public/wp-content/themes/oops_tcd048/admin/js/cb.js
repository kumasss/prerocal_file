jQuery(document).ready(function($){
	...
	$('#contents_builder').on('click', '.button-add-row', function(){
	...
	// リピーターフィールドの削除
	$('#contents_builder').on("click", ".button-delete-row", function(){
		var del = true;
	...
	// ショーケース
	$('#contents_builder').on('click', '.cb-showcase-type1, .cb-showcase-type2', function() {
		$(this).parents('.cb_content').find('.cb-showcase-image').show();
	});
	$('#contents_builder').on('click', '.cb-showcase-type3', function() {
		$(this).parents('.cb_content').find('.cb-showcase-image').hide();
	});
