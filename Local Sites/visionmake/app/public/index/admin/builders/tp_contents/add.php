...
elseif(isset( $message )) {
	...
	echo '<div class="waku"><p>コンテンツページを作成します。保存するとページのアドレスが決まります。</p></div>';
}
?>
...
if(!empty($sidebars_data)){
	foreach($sidebars_data as $col){
		...
		echo ("<option value='".$col['title']."'".$selected.">".$midasi."</option>\n");
	}
}
?>
</select>
...
<input type="text" id="SideTitles" value="<?php echo $form_data['side_title2']; ?>" style="width:315px;" name="side_title2"><br>
...
<span class="help-inline">固定　　：保存すると決まります。</span>
</div>
</div>
...
<textarea id="ContentsContent" rows="25" class="span9" name="contents"><?php echo $form_data['contents']; ?></textarea>
...
<?php
if(!empty($form_data['eye_image_id'])){
	$buildersObj->get_img_uploaders( $form_data['eye_image_id'], $eye_image );
...
<div class="controls required">
<input type="txtarea" id="" class="input-mini" name="public_date" value="0">
<span style="padding:4px;">日後から</span>
<input type="txtarea" id="" class="input-mini" name="no_public_date">
...
<div class="controls">
<input type="txt" id="ContentsPassword" value="<?php echo $form_data['password']; ?>" class="input-small" name="password">
<label class="green">※このページにパスワードをかけたい場合に設定してください</label>
</div>
</div>
