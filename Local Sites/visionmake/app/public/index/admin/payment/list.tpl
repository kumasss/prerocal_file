...
<?php foreach ($records as $item): ?>
									<tr>
										...
										<td>
											<a href="#{URL}/purchase/?id=#{$item->id}" target="_blank">#{$item->shortTitle()}</a>
											<br/>#{$item->id}
	<ol>
	<?php foreach ($item->getUnits() as $i=>$unit): ?>
		<li><a class="popup" href="product_item.php?pid=#{$unit->id}">#{$unit->shortTitle()}</a></li>
	<?php endforeach ?>
	</ol>
										</td>
										<td>#{$item->price}円</td>
										...
										<td style="text-align: center">
											<a class="btn btn-primary edit"	 type="button" href="#{$item->id}">編集</a>
											<a class="btn btn-default action" type="button" href="#{$item->id}" name="copy">複製</a>
											　　
<?php if( $item->total_contract>0) : ?>
											　　　　
<?php else: ?>
											<a class="btn btn-danger action" type="button" href="#{$item->id}" name="delete">削除</a>
<?php endif; ?>
										</td>
									</tr>
<?php endforeach ?>
