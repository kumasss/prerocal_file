...
<div class="blogs form">
	...
	<div style="display:none;"><input type="hidden" value="" name="id"></div>
	<fieldset>
		...
		<select id="BlogUpdateHour" class="input-mini" name="send_time_hour">
			<?php for($h=0;$h<24;$h++): ?>
				<option value=<?php echo sprintf("%02d",$h);?> <?php echo(sprintf("%02d",$h)==$form_data['send_time_hour'])?"selected":"";?>><?php echo $h;?></option>
		    <?php endfor; ?>
		</select>時
		<select id="BlogUpdateMin" class="input-mini" name="send_time_minute">
			<?php for($m=0;$m<60;$m+=10): ?>
				<option value=<?php echo sprintf("%02d",$m)?> <?php echo(sprintf("%02d",$m)==$form_data['send_time_minute'])?"selected":"";?>><?php echo $m;?></option>
		    <?php endfor; ?>
		</select>分
		</div>
		</div>
