...
?>
		...
		<input type="txt" id="NameEmail" value="<?php echo $form_data['firstname']; ?>" class="input-large" name="firstname">
...
<?php */?>
		...
		<div class="help-block"><span class="bold">設定例）</span><br>
<span class="green bold">エックスサーバー</span>：1500通/1時間の制限なので余裕を持たせて<span style="font-weight:bold;color:#dd0000;">[1通/6秒]</span>と設定をしてください。<br>
<span class="green bold">さくらレンタルサーバー</span>：250通/15分の制限なので余裕を持たせて<span style="font-weight:bold;color:#dd0000;">[1通/8秒]</span>と設定をしてください。<br>
なおサーバーの方の送信制限はサーバー全体での制限となります。Cyfonsを複数設置されている場合はご注意ください。
		</div>
		</div>
		</div>
		<div class="control-group">
		<label class="control-label" for="optionsCheckbox">初回即時送信</label>
		<div class="controls">
		<label class="checkbox" for="SendNow">
		<input type="hidden" name="send_now" value="0">
		<input type="checkbox" name="send_now" id="SendNow" value="1" <?php if($form_data['send_now'] == 1) echo "checked"; ?>>ステップメールのストーリNo１を会員登録時に即時送信する
		</label>
		</div>
		</div>
<?php /* ?>
		...
		<input type="checkbox" name="automail_add_admin" id="AutomailAddAdmin" value="1" <?php if($form_data['automail_add_admin'] == 1) echo "checked"; ?>>会員登録フォームからの登録時
		</label>
		<label class="checkbox" for="AutomailStopAdmin">
		<input type="hidden" name="automail_stop_admin" value="0">
		<input type="checkbox" name="automail_stop_admin" id="AutomailStopAdmin" value="1" <?php if($form_data['automail_stop_admin'] == 1) echo "checked"; ?>>会員解除フォームからの解除時
		</label>
		<label class="checkbox" for="AutomailEditAdmin">
		<input type="hidden" name="automail_edit_admin" value="0">
		<input type="checkbox" name="automail_edit_admin" id="AutomailEditAdmin" value="1" <?php if($form_data['automail_edit_admin'] == 1) echo "checked"; ?>>会員情報修正ページで修正時
		</label>
		</div>
		</div>
		<div class="control-group">
		...
		<input type="checkbox" name="automail_add_user" id="AutomailAddUser" value="1" <?php if($form_data['automail_add_user'] == 1) echo "checked"; ?>>会員登録フォームからの登録時
		</label>
		<label class="checkbox" for="AutomailStopUser">
		<input type="hidden" name="automail_stop_user" value="0">
		<input type="checkbox" name="automail_stop_user" id="AutomailStopUser" value="1" <?php if($form_data['automail_stop_user'] == 1) echo "checked"; ?>>会員解除フォームからの解除時
		</label>
		<label class="checkbox" for="AutomailEditUser">
		<input type="hidden" name="automail_edit_user" value="0">
		<input type="checkbox" name="automail_edit_user" id="AutomailEditUser" value="1" <?php if($form_data['automail_edit_user'] == 1) echo "checked"; ?>>会員情報修正ページで修正時
		</label>
		</div>
		</div>
...
<?php */ ?>
	...
	<button type="submit" class="btn btn-primary" style="margin-right:5px">保存</button><a class="btn" href="<?php echo URL; ?>/admin/mails/">キャンセル</a>
	</div>
	</form>
</div>
</body>
