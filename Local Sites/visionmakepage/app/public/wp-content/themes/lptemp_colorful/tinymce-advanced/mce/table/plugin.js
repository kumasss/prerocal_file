	...
	return function(editor) {
		...
		function updateStyle(dom, win, isStyleCtrl) {
			...
			if (isStyleCtrl) {
				...
				win.find('#backgroundImage').value(css["background-image"] || '')[0].fire('change');
			} else {
				css["border-color"] = data.borderColor;
				css["background-color"] = data.backgroundColor;
				css["background-image"] = data.backgroundImage;
			}
		...
		function appendStylesToData(dom, data, elm) {
			...
			if (css["background-image"]) {
				data.backgroundImage = css["background-image"];
			} else {
				data.backgroundImage = 'url( ここに取得した画像URLをペースト )';
			}
