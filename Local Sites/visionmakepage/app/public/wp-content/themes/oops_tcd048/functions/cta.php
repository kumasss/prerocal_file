...
function tcd_cta_scripts() {
	...
	wp_enqueue_script( 'oops-cta', get_template_directory_uri() . '/admin/js/cta.min.js', array( 'jquery' ), version_num(), true );
	wp_localize_script( 'oops-cta', 'tcd_cta', array( 'admin_url' => admin_url( 'admin-ajax.php' ), 'ajax_nonce' => wp_create_nonce( 'tcd_cta_nonce' ) ) );
...
function tcd_admin_cta_scripts() {
	...
	wp_enqueue_script( 'oops-cta', get_template_directory_uri() . '/admin/js/cta.min.js', array( 'jquery' ), version_num(), true );
	wp_localize_script( 'oops-cta', 'tcd_cta', array( 'admin_url' => admin_url( 'admin-ajax.php' ), 'ajax_nonce' => wp_create_nonce( 'tcd_cta_nonce' ), 'confirm_text' => __( 'Are you sure to reset these values?', 'tcd-w' ) ) );

}
add_action( 'admin_print_scripts-appearance_page_theme_options', 'tcd_admin_cta_scripts' );
...
for ( $i = 1; $i <= 3; $i++ ) {
	switch ( $options['cta_type' . $i] ) :
		case 'type1' :
?>
.p-cta--<?php echo $i; ?>::before { background: <?php echo esc_html( $options['cta_type1_overlay' . $i] ); ?>; opacity: <?php echo esc_html( $options['cta_type1_overlay_opacity' . $i] ); ?>; }
.p-cta--<?php echo $i; ?> .p-cta__btn { background: <?php echo esc_html( $options['cta_type1_btn_bg' . $i] ); ?>; }
.p-cta--<?php echo $i; ?> .p-cta__btn:hover { background: <?php echo esc_html( $options['cta_type1_btn_bg_hover' . $i] ); ?>; }
<?php
			break;
		case 'type2' :
?>
.p-cta--<?php echo $i; ?> .p-cta__btn { background: <?php echo esc_html( $options['cta_type2_btn_bg' . $i] ); ?>; }
.p-cta--<?php echo $i; ?> .p-cta__btn:hover { background: <?php echo esc_html( $options['cta_type2_btn_bg_hover' . $i] ); ?>; }
<?php
			break;
	endswitch;
}
